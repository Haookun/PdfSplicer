...
PdfSplicerはその悩みを解決します！macOS向けのスマートPDF結合ツールで、モダンなCustomTkinterインターフェースとドラッグ&ドロップ対応、ページ順自動認識、ワンクリックで完全なPDFに統合します。

## 主な機能
- モダンなCustomTkinterインターフェース、シンプルで美しい
- ドラッグ&ドロップまたはクリックでPDFファイルを選択
- 表裏PDFの選択
- ページ順自動認識・結合
- 出力フォルダ選択・即時オープン
- ワンクリックでPDF生成
- 空白ページ自動スキップ：オプションで有効化可能。白色ピクセル比率分析により、スキャナーによるほぼ白いページを含む空白ページを自動判定して除外、効率アップ。
